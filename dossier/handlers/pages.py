    ...
    async def get(self, for_user=None, server_name=""):
        ...
        if isinstance(spawner, DossierKubeSpawner):
            if spawner.tenant is None:
                tenants = {
                    t["metadata"]["name"]: t
                    for t in utils.get_tenants()
                    if t["metadata"]["name"] in auth_state["tenants"]
                }
                if len(tenants) == 0:
                    if spawner.default_tenant_name:
                        if spawner.default_tenant_name in tenants:
                            ...
                            raise web.HTTPError(
                                ...
                                spawner.default_tenant_name,
                                self,
                            )
                    else:
                        raise web.HTTPError(
                            403, "User {} has no tenants assigned.", user.name, self
                        )
                elif len(tenants) == 1:
            ...
            if not hasattr(spawner, "confirmed"):
                spawners = {
                    s["metadata"]["name"]: s
                    for s in utils.get_spawners()
                    if spawner.tenant["metadata"]["name"]
                    in s["spec"].get("tenants", [])
                }
                if len(spawners) == 0:
